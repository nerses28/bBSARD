...
class Evaluator:
    def __init__(self, metrics_at_k: Dict[str, List[int]] = {'recall': [100, 200, 500], 'map': [100], 'mrr': [100], 'ndcg': [10, 100]}):
    ...
    def dcg(self, ground_truths: List[int], results: List[int], k: int = None):
        k = len(results) if k is None else k
        dcg_score = sum([1 / math.log2(i + 2) if results[i] in ground_truths else 0 for i in range(k)])
        return dcg_score

    def idcg(self, ground_truths: List[int], k: int):
        ideal_relevances = [1] * min(len(ground_truths), k)
    ...
    def ndcg(self, ground_truths: List[int], results: List[int], k: int = None):
        k = len(results) if k is None else k
        dcg_value = self.dcg(ground_truths, results, k)
        idcg_value = self.idcg(ground_truths, k)
